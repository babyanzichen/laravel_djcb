...
$(function() {
    ...
    function main() {
        ...
        initModel.getSongList("/index/radio/player/data/data.json", function(data) {
            // 生成所有歌曲列表
        ...
        function renderList(data, type, lsongArr, callback) {
                ...
                if (!sListTpl) {
                    ...
                    $.each(data, function(index, el) {
                        if (index >= songIndex && index < songIndex + Settings.reqNum) {
                            tpl += "<li class='song btm-line' data-src='/index/radio/player/res/music/" + songNum + ".mp3' data-index='" + songNum + "'><div class='poster'><img src='"+url+"player/img/poster/" + songNum + "-thumbnail.jpg'></div><div class='songinfo'><h2 class='lsongname'>" + el.songName + "</h2><sub class='lsinger'>" + el.singer + "</sub></div><div class='loveflag'><i class='icon icon-love '></i></div></li>";
                            songNum++;
                        }
                    });
            ...
            function renderLSongList(data, lsongArr) {
                ...
                if (lSongArr && lSongArr.length !== 0) {
                    ...
                    $.each(data, function(index, el) {
                        if (lSongArr[0] === index) {
                            tpl += "<li class='song btm-line' data-src='res/music/" + index + ".mp3' data-index='" + index + "'><div class='poster'><img src='index/radio/player/img/poster/" + index + "-thumbnail.jpg'></div><div class='songinfo'><h2 class='lsongname'>" + el.songName + "</h2><sub class='lsinger'>" + el.singer + "</sub></div><div class='loveflag'><i class='icon icon-love active'></i></div></li>";
                            lSongArr.shift();
                        }
                    });
                    $(container).append($(tpl));
    ...
    function EventHandler() {
        ...
        Dom.sliderWrap.eq(0).on("scroll", function(e) {
            ...
            if (Util.isScrollToBottom($(this), listBoxWinH, scrT)) {
                updateSongModel.getSongList("/index/radio/player/data/data.json", function(data) {
                    // 生成歌曲列表
        ...
        // 点击一首歌曲
        $(".g-songlist").on("click", ".song", function(e) {
            e.preventDefault();
            e.stopPropagation();
            ...
            // 音频相关
            if (Dom.audio.src == rootPath + $(this).data("src")) {
                return;
            }
            Dom.audio.src = $(this).data("src");
            Dom.audio.play();
            ...
            // 播放视图相关
            songInfo = {
                songname: $(this).find(".lsongname").text(),
                singer: $(this).find(".lsinger").text(),
                lrcimg:url+"player/img/poster/" + $(this).data("index") + "-origin.jpg",
                lyric: Util.getItem("lyric" + $(this).data("index"))
            };
        ...
        Dom.prev.on("click", function() {
            ...
            songInfo = {
                ...
                lrcimg: url + "player/img/poster/" + lrcIndex + "-origin.jpg",
