...
<table class="table table-hover">
   <caption>品牌链接列表</caption>
   <thead>
      <tr> 
      <th class="companys">编号</th>
      <th class="companys">品牌名</th>
      <th>品牌logo</th>
      <th>商城链接</th>  
      <th>一级分类</th>  
      <th>二级分类</th>  
   ...
   <tbody>
      ...
      <tr>
        ...
        <td> {{$brand->shoplink}}</td>
         <td> {{$brand->category1}}</td>
          <td> {{$brand->category2}}</td>
           <td> {{$brand->counts}}</td>
         <td>{{$brand->created_at}}</td>
         <td>{{$brand->updated_at}}</td>
          <td><button onclick="edit({{$brand->id}})">修改</button></td>
      </tr>
      @endforeach 
     <tr class="content">
   ...
   function edit(id){
      ...
      success:function(data){
       var obj = JSON.parse(data);
        ...
        $("input[name=brandlogo]").val(obj.brandlogo);
        $("select[name=s1]").val(obj.category1);
        $("select[name=s2]").val(obj.category2);
        $("input[name=shoplink]").val(obj.shoplink);
      },
