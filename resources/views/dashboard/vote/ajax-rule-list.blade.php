        ...
        @forelse( $lists as $v )
            <tr class="long-td">
                ...
                <td>
                    ...
                    @else
                    <i class="fa fa-check text-navy change-status hover-point" data-value="no" data-cv="yes" data-id="{{ $v->id }}" data-column="is_enabled" data-table="vote_rules" data-msg="非启用" data-todo="0" data-cur="启用" onclick="changeStatus(this)">启用</i>
                                            @endif
                </td>
                <td>{{ $v->updated_at }}</td>
                <td>
